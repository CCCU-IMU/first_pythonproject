...
out_dir <- "sorted_Q"
out_prefix <- "admixture_ordered"

# 自定义群体顺序
custom_group_order <- c("Angus","Simmental","Charolais","UW","Mo-OD","Mo-SN","Hanwoo","indicus")
...
for(f in q_files){
  ...
  # 添加 Sample 和 Group
  q <- cbind(q, Sample = sample_info$Sample[match(sample_order, sample_info$Sample)],
...
group_breaks <- sample_info %>%
  mutate(pos = match(Sample, sample_order)) %>%
  group_by(Group) %>%
...
p <- ggplot(plot_data, aes(x=Sample, y=Proportion, fill=Cluster)) +
  ...
  scale_x_discrete(labels = plot_data$Group[!duplicated(plot_data$Sample)], expand=c(0,0)) +
